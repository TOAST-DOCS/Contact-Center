...
### 필드
필드는 티켓 생성 및 처리 시 입력할 필드를 설정하는 메뉴입니다. Online Contact에서 제공하는 기본 필드 외에도 상담에 필요한 입력 항목을 직접 정의하여 구성할 수 있습니다.

필드는 **고객 필드**와 **상담원 필드**로 구분되며 다음과 같이 사용됩니다.

- 고객 필드: 헬프센터 내 문의하기 시 **고객이 입력하는 항목**을 의미합니다. 접수유형 카테고리별로 필드를 다르게 설정할 수 있으며, 설정한 내용은 헬프센터 문의하기 화면에 반영됩니다. 헬프센터 외의 문의는 상담원이 티켓 처리 시 입력할 수 있습니다.
...
**③** Online Contact에서 시스템 상 필요에 의해 자동으로 만들어진 시스템 필드는 **초기화** 버튼으로 나타나며, 클릭 시 초기 설정 값으로 복구합니다. 반면, **추가** 기능을 통해 생성된 사용자 필드는 **삭제** 버튼으로 나타나며 클릭 시 해당 필드 정보가 삭제됩니다.
...
- **②** 템플릿 제목
- **③** 내용: [전체 관리 → 계약 서비스 현황 → 계약 기본정보 설정] 메뉴에서 설정된 에디터 타입이 반영되며, 내용 작성 시 우측의 **치환 코드**를 활용하실 수 있습니다. **더블 클릭** 시 치환 코드가 본문에 추가되며, 티켓 처리 시 템플릿의 치환 코드 영역에 데이터가 직접 삽입되어 보다 편리하게 답변을 작성하실 수 있습니다.
