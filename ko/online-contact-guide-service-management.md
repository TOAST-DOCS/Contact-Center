...
### 템플릿 관리
템플릿은 자주 사용되는 **티켓 작성 양식을 미리 추가**하여 티켓을 신속하게 처리할 수 있는 기능입니다.
...
**②추가** 버튼을 클릭하면 **템플릿 선택 대화상자**가 나타나며 선택한 템플릿을 해당 처리유형에 신규로 추가할 수 있습니다.
...
**① 대표 계정**: Online Contact 대표 메일계정을 생성할 수 있습니다. 생성된 계정으로 문의 메일이 접수되면 해당 내용이 티켓으로 만들어져 상담원이 확인하여 처리할 수 있습니다.
...
**④ 메일 레이아웃**: 티켓이 처리되었을 때 자동으로 발송되는 메일 내용을 **헬프센터 언어별**로 작성할 수 있습니다. **치환코드**를 선택하면 에디터에 예약어가 입력되며, 메일 전송 시 해당 티켓의 정보로 치환되어 전송됩니다. 치환 코드 중 답변 내용(#{content}) 코드는 필수로 입력해야 합니다.
...
**금칙어**는 고객 문의에 대한 답변 처리 시 오탈자 또는 잘못된 안내사항이 답변에 포함되는 것을 방지하는 기능입니다.

**① 추가** 버튼을 클릭하여 금칙어를 등록할 수 있습니다.

**② 티켓 처리** 시 **미리보기** 또는 **티켓 해결/보류** 처리할 경우, 내용에 금칙어가 포함되면 **금칙어 대화상자**가 표시되어 티켓 처리를 통제합니다. 내용에 포함된 금칙어를 확인 후 수정하면 정상적으로 티켓을 처리할 수 있습니다.

## 전화

**전화** 메뉴에서는 콜백 관리를 위한 **IVR 경로 코드, 경로명**을 등록하고, 전화 발신 시 고객에게 표시되는 **발신번호**를 등록할 수 있습니다. 해당 메뉴는 계약 정보에서 **티켓 관리 → 전화CTI사용 기능**이 **사용**으로 설정되어야 표시됩니다.
...
**② 전체 발신번호**는 **추가** 기능을 통해 등록된 모든 발신번호 내역이 표기됩니다. 

**③** 발신번호를 추가하려면 **추가** 버튼을 클릭합니다. 발신번호 추가 화면으로 이동되며 발신번호 이름, 발신번호, 통신 서비스 이용 증명원을 첨부한 후 저장하면 **'확인중'** 상태로 등록됩니다. 이후 서비스 이용 증명원 검증이 완료되면 **'등록완료'** 또는 **'등록불가'** 상태로 변경되며, 상태 변경에 대한 사유를 확인할 수 있습니다. **등록 완료**된 발신번호는 등록 완료 목록에 추가됩니다.
