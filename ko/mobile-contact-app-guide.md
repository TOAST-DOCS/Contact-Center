...
2. 검색창에 ‘https://mobile-contact.toast.com/download’를 입력하세요.
...
- 대기: 상담전화 착신이 활성화된 상태
- 휴식, 후처리: 상담전화 착신이 비활성화된 상태
- 다이얼패드: 번호 입력 후 발신 가능 (PC의 다이얼 패드는 메인화면에 위치합니다.)
- 로그아웃: 상담 업무 종료 시 로그아웃

### 상담 전화 받기
① 개인 전화와 구분되는 수신화면을 제공하며, 상담전화가 수신되면 버튼을 오른쪽으로 밀어 통화를 수락할 수 있습니다. (PC, AOS는 통화버튼을 누르면 통화가 수락됩니다.)
② 상담전화가 시작되면 통화 정보 및 고객정보를 확인할 수 있습니다.
③ 상담전화를 종료하려면 종료 버튼을 누르세요.

- 상담전화 중 통화 정보는 착신∙발신 종류 및 상담업무, 최근 통화목록(5개)이 표시됩니다.
- 기기에 따라 제조사에서 제공되는 수신화면이 나타날 수 있습니다.
- 기기에 따라 내부스피커, 외부스피커, 블루투스를 지원할 수 있습니다.
...
- 상담 전화 걸기는 내선 또는 국선으로 통화 연결이 가능합니다.
- 서비스에 따라 국제전화에 대해서는 지원하지 않습니다.   
...
### 후처리 하기
① 상담 전화가 종료되면 상담 이력을 저장할 수 있는 후처리 화면으로 이동합니다.
② 상담 전화에 관련된 정보, 날짜, 시간을 확인 후 상담업무를 지정, 문의/조치내용을 입력하세요.
③ 후처리 내용 입력 후 대기 또는 휴식 버튼을 선택하세요.

- 문의/조치내용은 1024자 까지 저장 가능합니다.
- 휴식, 대기 버튼 선택 후 메인화면 이동 시 상담사 상태가 휴식, 대기상태로 반영됩니다.
- 통화 종료 후 입력한 상담내용은 휴식 또는 대기 버튼을 누르면 자동 저장됩니다.
- 통화 종료 후 상담내용을 바로 입력하지 않아도 입력 가능한 시간에 언제든지 메인화면 > 상담이력에서 다시 저장할 수 있습니다. (이미 저장된 경우라도 재수정 가능)

## 메인화면
메인화면에서는 상담 업무와 관련된 다양한 기능을 확인할 수 있으며, 데이터를 실시간으로 표시합니다.

- 상담사 상태 설정/소요시간 표기 (대기, 휴식, 후처리, 다이얼패드, 로그아웃)
- 고객 대기 : 고객 대기 현황
- 응대율 : CTI큐 실시간 현황
- 그래프 : 인입∙응대호 수, 평균∙최대 대기 (통합, 전일, 당일) 
- 인입호 수 ∙응대호 수 : 당일 총 인입/응대호 수
- 평균 대기 ∙최대 대기 : 대기자의 평균 ∙ 최대 대기 시간
- 상담사 상태 : 상담사 실시간 현황
- 개인 : 상담사 통계
- 상담이력, 호처리통계, 콜백, 더보기
...
## 상담사 상태/개인 통계
### 상담사 상태 (상담사 실시간 현황)
로그인 중인 모든 상담사의 실시간 현황을 확인할 수 있습니다.

- 그래프 : 상담사 상태 현황
- 상담사 : 상담사 이름
- 인입∙응답 : 당일 총 인입 수∙당일 총 응답 수
- 응답률 : 당일 총 응답률

### 개인 (상담사 통계) 
상담사 통계에서 날짜를 선택하여 검색 버튼을 누르면 상담사 개인의 상담 통계를 실시간으로 확인할 수 있습니다. 

- 이름 : 상담사 이름
- 일자 : 상담업무 날짜
- 응답∙통화 : 당일 총 응답수∙당일 총 통화수
- 내선∙총통화 : 당일 총 내선 통화 수∙당일 총 통화 시간
