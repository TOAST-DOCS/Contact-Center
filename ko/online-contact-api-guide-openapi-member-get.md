...
3. 헬프센터에서 ‘토큰 검증 URL’을 호출합니다. (여기서 ‘토큰 검증 URL 은 고객사에서 아래 제공된 명세서에 따라 개발 후 OC 회원연동 화면에 등록해야 합니다.)
4. 토큰 검증 후 정상일 경우 ‘문의하기’ 또는 ‘문의내역’ 페이지에 접속합니다. 이 때 검증에 실패 할 경우 ‘문의하기’는 비회원으로 접수되며 ‘문의 내역’에서는 ‘에러’ 페이지로 랜딩처리됩니다.
